    ...
    <motion.article
      ...
      className={`group cursor-pointer rounded-2xl overflow-hidden glass border transition-all duration-300 ${
        isSelected
          ...
          : 'border-white/10 hover:border-primary-500/30 hover:shadow-glow-sm'
      }`}
    >
      <motion.div className="relative aspect-video overflow-hidden">
        ...
        <span className="absolute top-2 left-2 px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wide bg-primary-500/90 text-white flex items-center gap-1">
        ...
        <motion.div
          ...
          initial={false}
        >
          <motion.div className="w-12 h-12 rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center border border-white/30">
            <Play className="w-5 h-5 text-white fill-white ml-0.5" />
          </motion.div>
        </motion.div>
      </motion.div>
      <motion.div className="p-3">
        <h3 className="font-medium text-sm line-clamp-2 group-hover:text-primary-300 transition-colors">
          {video.title}
        </h3>
...
export const VideoCardSkeleton = () => (
  <motion.div className="rounded-2xl overflow-hidden glass border border-white/10 animate-pulse">
