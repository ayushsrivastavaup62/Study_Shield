...
const StudyTimer = ({ compact = false, onPresetChange }) => {
  ...
  return (
    <motion.div
      ref={dropdownRef}
      className={`relative flex items-center gap-2 glass rounded-full border border-white/10 ${compact ? 'px-3 py-1.5' : 'px-4 py-2'}`}
      whileHover={{ boxShadow: '0 0 24px rgba(56, 189, 248, 0.15)' }}
    >
      <motion.div
        key={resetKey}
      ...
      >
        ...
        <Clock
          className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-primary-400 ${compact ? 'w-3 h-3' : 'w-3.5 h-3.5'}`}
        />
      </motion.div>
      ...
      <button
        ...
        className="p-1.5 rounded-full hover:bg-white/10 transition-colors ripple"
      ...
      <button
        ...
        className="flex items-center gap-0.5 p-1.5 rounded-full hover:bg-white/10 transition-colors border-l border-white/10 pl-2 ml-0.5"
      ...
      <AnimatePresence>
        {dropdownOpen && (
          <motion.div
            ...
            className="absolute top-full right-0 mt-2 w-48 glass-dark rounded-2xl border border-white/10 shadow-glow overflow-hidden z-50"
          >
            <motion.div className="p-2">
              <p className="text-xs text-gray-500 uppercase tracking-wider px-2 py-1">Presets</p>
              {PRESETS.map((preset) => (
                <button
                  key={preset.label}
                  type="button"
              ...
              <button
                ...
                className="w-full text-left px-3 py-2 rounded-xl text-sm hover:bg-white/10 transition-colors text-primary-400"
              >
                Custom timer
              </button>
              {showCustom && (
                ...
                >
                  <input
                    ...
                    className="flex-1 bg-white/5 border border-white/10 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:border-primary-500"
                  />
                  <button
                    type="button"
                    onClick={applyCustom}
